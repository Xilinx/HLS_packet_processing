...
#ifndef TEST
#define TEST test_prepend_set1
#endif

//using namespace mqttsn;

class test_prepend_set1 {
public:
...
class test_prepend_set5 {
public:
    static const int LENGTH = 22;
    static void test(stream<axiWord> &dataIn, stream<axiWord> &dataOut) {
        Packet p;
        header<Packet,5> ih(p);
        header<header<Packet,5>,2 > ic(ih);
        ipv4_hdr< header<header<Packet,5>,2 > > h(ic);
        // mqttsn_publish_hdr<Packet> ih(p);
        // mqttsn_hdr<mqttsn_publish_hdr<Packet> > ic(ih);
        // ipv4_hdr<mqttsn_hdr<mqttsn_publish_hdr<Packet> > > h(ic);
        ih.deserialize(dataIn);
        for(int i = 0; i < LENGTH; i++) {
            h.set<1>(i, i-(LENGTH));
        }
    ...
    for(int i = 40; i < 80; i++) {
        ...
        while (!(outFIFO.empty())) {
            ...
            */
            ...
            bool fail = false;
            for(int s = 0; s < S; s++) {
                if(outData.keep[s] && outData.data(8*s+7,8*s) != ap_uint<8>(count2)) {
                    fail = true;
                    errCount ++;
                }
                if(outData.keep[s]) {
