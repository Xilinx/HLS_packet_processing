...
void dumpDataBeats32(hls::stream<ap_uint<32> > &stream) {
    dumpDataBeats(stream);
}

void configSource(hls::stream<ap_uint<64> > &config) {
    static int done = false;
    if(!done) {

        config.write(destMAC);
        config.write(destIP);
        config.write(destPort);
        done = true;
    }
}
void packetSource(    hls::stream<StreamType> &inp) {
#pragma HLS inline all recursive
    {
        ...
        std::cout << mh << "\n";
        eh.serialize(inp);
        //process_packet(destMAC, destIP, destPort, inp, output, outputMeta);
...
int main(int argc, char* argv[])
{
    ...
    packetSource(input);
    configSource(config);
    process_packet_config(config, input, /*output,*/ outputMeta);
    process_packet_config(config, input, // output,
                          outputMeta);
    process_packet_config(config, input, // output,
                          outputMeta);
    dumpDataBeats(outputMeta);
